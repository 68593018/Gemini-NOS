...
nos_status_t nos_scheduler_init_thread(nos_thread_t *thread, uint32_t id, const char *name) {
    ...
    ev.events = EPOLLIN;
    ev.data.fd = thread->notify_fd[0]; // 唤醒管道使用 data.fd
...
/* 本地传输：Buffer 所有权转移 */
static nos_status_t nos_transport_local_send(nos_buffer_t *buf) {
    nos_thread_t *target_thread = g_main_thread_ptr; 
...
static nos_status_t nos_transport_remote_uds_send(nos_buffer_t *buf, const char *uds_path) {
    ...
    while (retry >= 0) {
        ...
        printf("[Transport] Connection to %s failed (err:%s). Retrying...\n", uds_path, strerror(errno));
...
nos_status_t nos_service_msg_send(nos_buffer_t *buf) {
    ...
    return nos_transport_local_send(buf);
}
    ...
    while (1) {
        ...
        if (header->dst_service != 0) {
            ...
            for (uint32_t i = 0; i < self->component_count; i++) {
                if (self->components[i]->id == 1) { // 演示简化
