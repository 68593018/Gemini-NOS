...
static const nos_platform_init_func_t g_infra_inits[] = { nos_kv_db_init ,nos_timer_init ,nos_log_init , NULL };
const nos_node_def_t g_local_node_def = {
    .name = "ProcA", .uds_path = "/tmp/nos_proc_A.sock", .buffer_pools = g_local_pools,
    .threads = {
        { .name = "Worker-1", .comp_ids = {1, 3, 0}, .comp_names = {"Comp-1", "Comp-3", NULL}, .comp_models = {"libcomp-1.so", "libcomp-3.so", NULL} },
        { .name = "Worker-2", .comp_ids = {2, 5, 0}, .comp_names = {"Comp-2", "Comp-5", NULL}, .comp_models = {"libcomp-2.so", "libcomp-2.so", NULL} },
        { .name = "Worker-3", .comp_ids = {10, 0}, .comp_names = {"Comp-Ping", NULL}, .comp_models = {"libcomp-ping.so", NULL} },
        { .name = "Worker-4", .comp_ids = {11, 0}, .comp_names = {"Comp-Pong", NULL}, .comp_models = {"libcomp-pong.so", NULL} },
        { .name = NULL }
    },
    .services = g_local_services, .service_count = 10, .platform_inits = g_infra_inits
};
